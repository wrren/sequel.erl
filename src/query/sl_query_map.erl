...
%% public interface
-export([generate_modules/0, get/1, load_dir/1,
	 load_file/1, load_file/2, put/2, start_link/0, stop/0]).
...
%%
-spec load_dir(string()) -> {ok, [query_id()]} |
			    {error, term()}.

load_dir(Path) -> load_dir(Path, []).

-spec load_dir(string(), [atom()]) -> {ok,
				       [query_id()]} |
				      {error, term()}.
...
load_dir(Path, Prefix) ->
    case file:list_dir(Path) of
      {ok, Filenames} ->
	  {ok,
	   lists:usort(lists:flatten(lists:filtermap(fun
						       (Filename) ->
							   case
							     ...
							     {ok,
							      #file_info{type =
									     directory}} ->
								 ID =
								     list_to_atom(filename:rootname(Filename)),
								 case
								   load_dir(filename:join(Path,
											  Filename),
									    [ID
									     | Prefix])
								     of
...
%%
-spec load_file(string()) -> {ok, query_id()} |
			     {error, term()}.
...
%%	given ID
%%
-spec load_file(string(), query_id()) -> {ok,
					  query_id()} |
					 {error, term()}.
...
%%
-spec generate_modules() -> {Loaded :: [module()],
			     Failed :: [{module(), term()}]}.

generate_modules() ->
    Map = ets:foldl(fun ({{Mod, Fun}, SQL}, Modules) ->
			    Module = maps:get(Mod, Modules, sl_module:new(Mod)),
			    maps:put(Mod, sl_module:add_query(Module, Fun, SQL),
				     Modules);
			(_, Modules) -> Modules
		    end,
		    maps:new(), ?MODULE),
...
handle_call({put, Query, SQL}, _From,
