...
-export([connect/1, connect/2, execute/2, execute/3,
	 prepare/2, prepare/3, query/1, query/2]).
...
%%
connect(Args) -> connect(?DEFAULT_POOL, Args).

%%
%%  @doc Connect to the database using the provided arguments. This function will create
%%  a connection pool with the specified name.
%%
connect(PoolName, Args = #{engine := Engine})
...
%%
connect(PoolName, EngineMod, Args)
    when is_atom(PoolName), is_atom(EngineMod),
	 is_map(Args) ->
    sl_worker_pool:add_pool(PoolName,
			    [{name, {local, PoolName}},
			     {worker_module, sl_worker},
			     {size, maps:get(pool_size, Args, 20)},
			     {max_overflow, maps:get(max_overflow, Args, 20)}],
			    [EngineMod, Args]).
...
%%  @doc Prepare the given query and associate it with the provided prepared statement name
%%
prepare(PoolName, Statement, SQL)
    when is_atom(PoolName), is_list(SQL) ->
...
%%
query(SQL) -> query(?DEFAULT_POOL, SQL).
...
query(PoolName, SQL) ->
    poolboy:transaction(PoolName,
			fun (Worker) -> sl_worker:query(Worker, SQL) end).
...
execute(PoolName, Statement, Args) when is_list(Args) ->
    case {sl_statement_map:get(PoolName, Statement),
	  sl_query_map:get(Statement)}
	of
      {undefined, undefined} -> {error, undefined_statement};
      {undefined, SQL} ->
	  poolboy:transaction(PoolName,
			      fun (Worker) ->
				      sl_worker:prepare(Worker, Statement, SQL),
